...
@Component
class PasswordResetEmailStrategy(
    private val objectMapper: ObjectMapper,
    private val emailSender: EmailSender,
) : EmailStrategy {
    override fun emailTemplate() = EmailTemplate.PASSWORD_RESET

    override fun handle(payload: String) {
        val command = objectMapper.readValue<PasswordResetEmailCommand>(payload)

        emailSender.send(
            to = command.email,
            subject = emailTemplate().subject,
            templatePath = emailTemplate().templatePath,
            variables = mapOf("resetLink" to command.resetLink),
        )
    }
}
